...
bool read_grid(const char* name, grid_type* data)
{
	std::ifstream file;
	std::istream* in = 0;
	if (!name || stricmp(name, "CON") == 0 || stricmp(name, "CON$") == 0 \
		|| stricmp(name, "CON:") == 0 || stricmp(name, "CONIN$") == 0)
	{
		if (-1 == _setmode(_fileno(stdin), _O_BINARY))
		{
			std::cerr << "cannot set binary mode for standard input stream\n";
			return false;
		}
		in = &std::cin;
	}
	else
	{
		...
		if (!file)
		{
			std::cerr << "failed to open grid file\n";
			return false;
		}
		in = &file;
	}
  ...
  if (strncmp(ch, "DSBB", 4) == 0)
  {
    ...
    if (!in->read((char*) data->zz, nn * sizeof(float)))
    {
    	return false;
    }
  }
  ...
  {
    if (*in >> data->cx >> data->cy >> data->x1 >> data->x2 >> \
