...
void compute_conflict_graph(LabelProblem* pflp)
{
  int n = pflp->numSites;
  int j = 4 * n;
  ...
  {
    j = i;
    LabelSite* p = pflp->site[i];
    while (++j < n)
    {
      if (!rr_intersect(&bb[i*4], &bb[j*4]))
        continue;
      LabelSite* q = pflp->site[j];
      ++p->degree;
      ++q->degree;
    }
    p->cfl = new LabelSite* [p->degree];
    p->degree = 0;
  }
...
int setup_initial_state(LabelProblem* pflp)
{
  int n = pflp->numSites;
  for (int i = 0; i < n; ++i)
  {
...
int resolve_conflict(LabelSite* p)
{
  double dx = p->bbox[2] - p->bbox[0];
  double dy = p->bbox[3] - p->bbox[1];
  double ss = 2.0 * M_PI * p->radius;
...
bool ss_compare(const LabelSite* p, const LabelSite* q)
{
  return (p->cost > q->cost);
}

bool cs_compare(const LabelSite* p, int cost)
{
  return (p->cost > cost);
}

void schedule_annealing(LabelProblem* pflp)
{
  // double t = -1.0 / log(1.0 / 3.0);
  srand(time(0));
  // PrintMessage(LEVEL_VERBOSE, __FILE__, __LINE__, "setting up an initial state\n");
  int total = setup_initial_state(pflp), n = pflp->numSites;
  if (total <= 0)
    return;

  for (int i = 0; i < 20*pflp->numSites; ++i)
  {
    std::sort(pflp->site, pflp->site+n, ss_compare);
    int nn = std::lower_bound(pflp->site, pflp->site+n, 0, cs_compare) - pflp->site;
    // std::cout << total << ' ' << nn << '\n';
    total += resolve_conflict(pflp->site[rand() % nn]);
    if (total <= 0)
      break;
  }
}

void graphics_output(psstream& out, LabelProblem* pflp)
{
  int n = pflp->numSites;
  for (int i = 0; i < n; ++i)
  {
...
int main()
{
  ...
  data.site = new LabelSite* [numSites];
  out.setgray(1.0);
  numSites = 0;
...
void driver()
{
  ...
  LabelProblem data;
  data.numSites = 3;
  data.site = new LabelSite* [3];

  LabelSite* s = new LabelSite;
  s->xcoord = 100;
  s->ycoord = 100;
  strcpy(s->text, "first");
